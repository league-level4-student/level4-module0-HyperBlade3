	...
	public WorldPanel(int w, int h, int cpr) {
		...
		for (int i = 0; i < cells.length; i++) {
			for (int j = 0; j < cells[0].length; j++) {
				...
				cells[i][j] = new Cell(j * (w / cpr), i * (h / cpr), cellSize);
	...
	public void mousePressed(MouseEvent e) {
		...
		for (int i = 0; i < cells.length; i++) {
			for (int j = 0; j < cells[0].length; j++) {
				...
				if ((e.getX() >= cells[i][j].getX() && e.getX() < cells[i][j].getX() + (int) Math.sqrt((double) cellSize))
						&& (e.getY() <= cells[i][j].getY() + (int) Math.sqrt((double) cellSize) && e.getY() > cells[i][j].getY())) {
